...
export const project = defineType({
  ...
  fields: [
    ...
    defineField({
      name: 'storyBlocks',
      title: 'Project story / case-study blocks',
      type: 'array',
      group: 'advanced',
      description: 'Optional. Add 2–4 short blocks that explain the thinking behind the work. Labels are fully editable — e.g. Context, My role, Creative move, Aha moment, Result.',
      of: [{
        type: 'object',
        name: 'storyBlock',
        fields: [
          defineField({name: 'label', title: 'Label', type: 'string', validation: (r) => r.required().max(40)}),
          defineField({name: 'text', title: 'Copy', type: 'text', rows: 4, validation: (r) => r.required().max(700)}),
        ],
    ...
    defineField({
      ...
      title: 'Related work (optional manual order)',
      type: 'array',
      group: 'advanced',
      description: 'Choose up to 3 projects to feature at the bottom. Leave empty and the website chooses automatically.',
